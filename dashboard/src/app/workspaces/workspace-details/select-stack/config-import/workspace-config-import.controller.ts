...
export class WorkspaceConfigImportController {
  ...
  workspaceConfigOnChangeCallback: Function;

  /**
   * Default constructor that is using resource
   * @ngInject for Dependency injection
   */
  constructor($log: ng.ILogService, $scope: ng.IScope) {
    this.$log = $log;
    ...
    $scope.$on('workspaceConfigEvent:workspaceConfig', (event: ng.IAngularEvent, config: any) => {
      try {
        this.importWorkspaceJson = angular.toJson(config, true);
      } catch (e) {
        this.$log.error(e);
      }
    });
  ...
  onChange(): void {
    ...
    try {
      ...
      if (!this.validationError) {
        this.workspaceConfigOnChangeCallback({config: config});
      }
    } catch (e) {
